...
- 主窗口**日历视图**：月历、按「计划日期」浏览待办、日详情与快速添加
- 农历、节气、法定假日与调休补班展示
- 日历快速添加时可选择目标计划（偏好会记住）
- 新建事项默认带上创建当天的计划日期

### Changed

- 事项编辑：所属计划与计划日期布局优化；换计划需点保存才生效
- 计划列表主区域宽度自适应
- WebView 空白处右键菜单改为中文「重新加载」

## [1.4.6] - 2026-09-12

### Fixed

- Windows：快速添加首次开窗、文件/导出对话框改为后台线程，降低 WebView2 卡死风险
- Windows：主窗口与桌宠位置/尺寸统一为逻辑像素，修复高 DPI 下偏移与透明边
- Windows：关闭主窗口时提示「已最小化到后台，托盘可退出」
- Windows：收起桌宠时在 128px 安全区内居中，减少右侧透明空区
- 启动恢复桌宠与 quick-add 创建改为延迟主线程调度
